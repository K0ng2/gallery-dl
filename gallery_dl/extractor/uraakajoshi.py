...
LIST_PATTERN = rf"{BASE_PATTERN}/list/users-list(?:/page/(\d+))?"
...
class UraakajoshiListExtractor(UraakajoshiExtractor):
	"""Extractor for Uraaka-joshi user list"""

	subcategory = "list"
	pattern = LIST_PATTERN
	example = "https://www.uraaka-joshi.com/list/users-list/page/3/"

	def __init__(self, match):
		UraakajoshiExtractor.__init__(self, match)
		self.page_start = text.parse_int(match[1]) if match and match[1] else 1
	...
	def items(self):
		...
		for tweet_data in self.api.user_list(
			page_start=self.page_start, cursor=self._init_cursor()
		):
			user_data = tweet_data.get("user", {})
			username = user_data.get("screen_name")
...
class UraakajoshiAPI:
	...
	def user_timeline(self, username, max_pages=None):
		...
		return self._fetch(params, max_pages, page_start=1)
	...
	def _make_request(self, params, type="timeline"):
		"""Make API request and return parsed data"""
		url = f"{self.root}/json/{type}/"
	...
	def _fetch(self, base_params, max_pages=None, page_start=1, type="timeline"):
		...
		while True:
			...
			data = self._make_request(params, type)
	...
	def hashtag_timeline(self, hashtag, max_pages=None, page_start=1, cursor=None):
		...
		if cursor is not None:
			page_start = cursor
		return self._fetch(params, max_pages, page_start)

	def user_list(self, max_pages=None, page_start=1, cursor=None):
		"""Fetch user list timeline with pagination"""
		...
		if cursor is not None:
			page_start = cursor
		return self._fetch(params, max_pages, page_start, "list")
